...
for cont_experiment in range(1, n_experiments + 1):
    ...
    for n_of_rdds in range(n_rddas_min, n_rddas_max + 1):
        ...
        oCBN = CBN(n_of_rdds, n_of_variables_rdda, n_of_signals_rdda, n_exit_variables,
                   n_clauses_function)

        # Generate the RDDs
        print("generating the rdds ...")
        oCBN.generate_local_networks(type_network=type_network)

        # Calculate the Attractors by RDDA and by Signal
        v_begin_0 = time.time()
        result = CBN.find_attractors_rddas_ray.remote(oCBN)
        oCBN = ray.get(result)
        v_end_0 = time.time()
        ...
        # Calculate the Attractors by RDDA and by Signal
        v_begin_1 = time.time()
        result = CBN.calculation_compatible_pairs_ray.remote(oCBN)
        oCBN = ray.get(result)
        v_end_1 = time.time()
        ...
        v_begin_2 = time.time()
        result = CBN.assembly_attractor_fields_pruning_ray.remote(oCBN)
        oCBN = ray.get(result)
        v_end_2 = time.time()
        ...
        res_dict = {
            ...
            "n_rdds": n_of_rdds,
            "n_rdda_attractors": len(oCBN.d_global_rdda_attractor.items()),
            "t_find_attractors_method": v_time_0,
            "n_pair_attractors": len(oCBN.list_attractors_pairs),
            "t_comp_paris_method": v_time_1,
            "n_attractor_fields": len(oCBN.attractor_fields),
